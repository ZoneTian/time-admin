<template>
  <div class="detail-container">
    <el-breadcrumb separator="/" style="margin-bottom: 20px">
      <el-breadcrumb-item
        @click="goList"
        style="cursor: pointer; color: #3a5bdb"
    ...
    <div class="action-buttons">
      <el-button type="primary" @click="editActivity" v-if="activity"
        >编辑活动</el-button
      >
    </div>
    ...
    <el-tabs v-model="activeTab" class="detail-tabs">
        ...
        <template v-if="activity">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item
              label="活动封面"
              :span="2"
              v-if="activity.mainCoverImage"
            >
              <div class="cover-in-table">
                <el-image
                  :src="activity.mainCoverImage"
                  fit="cover"
                  :preview-src-list="[activity.mainCoverImage]"
                ></el-image>
              </div>
            </el-descriptions-item>
            ...
            <el-descriptions-item label="报名时间">
              {{ activity.applyStartTime || "未设置" }} 至
              {{ activity.applyEndTime || "未设置" }}
            </el-descriptions-item>
            <el-descriptions-item label="城市">{{
              activity.city || "未设置"
            }}</el-descriptions-item>
            <el-descriptions-item label="活动地点">{{
              activity.address || "未设置"
            }}</el-descriptions-item>
            <el-descriptions-item label="活动费用"
              >¥{{ activity.price?.toFixed(2) || "0.00" }}</el-descriptions-item
            >
            ...
            <el-descriptions-item label="创建时间">{{
              activity.createTime || "未知"
            }}</el-descriptions-item>
          </el-descriptions>

          <div
            class="cover-images"
            v-if="activity.coverImages && activity.coverImages.length > 0"
          >
            <h3>活动封面图片</h3>
            <div class="image-grid">
              <div
                v-for="(image, index) in activity.coverImages"
                :key="index"
                class="image-item"
              >
                <el-image
                  :src="image"
                  fit="cover"
                  :preview-src-list="activity.coverImages"
                ></el-image>
              </div>
            </div>
          </div>
          ...
          <div
            class="detail-images"
            v-if="activity.detailImages && activity.detailImages.length > 0"
          >
            <h3>活动详情图</h3>
            <div class="image-grid">
              <div
                v-for="(image, index) in activity.detailImages"
                :key="index"
                class="image-item"
              >
                <el-image
                  :src="image"
                  fit="cover"
                  :preview-src-list="activity.detailImages"
                ></el-image>
              </div>
            </div>
          </div>
      ...
      <el-tab-pane label="报名成员" name="members">
        ...
        >
          ...
          <el-table-column prop="actualPrice" label="需付金额" width="100">
...
const fetchActivityDetail = async () => {
  ...
  try {
    ...
    const response = await axios.get(
      `${BASE_API_URL}${API_PATHS.ACTIVITY.DETAIL}`,
      {
    ...
    if (response.data && response.data.code === 0) {
      ...
      activity.value = {
        ...
        date: `${formatDate(activityData.activityStartTime)} - ${formatDate(
...
const getActivityStatus = (activityData: any) => {
  ...
  const startTime = activityData.activityStartTime
    ? new Date(activityData.activityStartTime).getTime()
    : 0;
  const endTime = activityData.activityEndTime
    ? new Date(activityData.activityEndTime).getTime()
    : 0;
  const signupDeadline = activityData.signupEndTime
    ? new Date(activityData.signupEndTime).getTime()
    : 0;
...
const fetchActivityMembers = async () => {
  ...
  try {
    ...
    const response = await axios.get(
      `${BASE_API_URL}${API_PATHS.ACTIVITY.REGISTRATION_LIST}`,
      {
        params: {
          activityId: id,
    ...
    if (response.data && response.data.code === 0) {
      ...
      activityMembers.value = registrationData.list.map((item: any) => ({
        ...
        useCouponFlag: item.useCouponFlag || false,
      }));
    } else {
...
const activityPhotos = ref([
  {
    url: "https://placeholder.pics/svg/300x200/DEDEDE/555555/活动照片1",
    description: "活动开场",
  },
  {
    url: "https://placeholder.pics/svg/300x200/DEDEDE/555555/活动照片2",
    description: "小组讨论",
  },
  {
    url: "https://placeholder.pics/svg/300x200/DEDEDE/555555/活动照片3",
    description: "合影留念",
  },
]);
...
// 新函数：使用router跳转到用户详情页面
const viewMemberDetail = (row: any) => {
  if (row && row.userId) {
    router.push(`/member/${row.userId}`);
  } else {
    console.error("用户数据不完整，无法查看详情", row);
    ElMessage.warning("用户数据不完整，无法查看详情");
  }
